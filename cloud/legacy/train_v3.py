"""GRPO inner loop v3 — broader detector ensemble in the reward.

Diff vs train_v2.py: reward averages over FOUR detectors, not two.
Tests the run #1 finding that a 2-detector reward overfit to the weaker
detector (roberta-base went +10pp ASR, roberta-large went -10pp). Adding
two more diverse detectors (DeBERTa-based desklib + fakespot RoBERTa-base)
should force the policy to find outputs that fool ALL four.

VRAM: +~2GB vs v2 (Desklib DeBERTa-v3-large ~1.7GB + Fakespot RoBERTa
~500MB). Drops G to 4 (was 6) to keep headroom on 24GB 4090.
...
class Cfg:
    ...
    # Multi-objective reward weights — quality-preserving emphasis.
    w_det: float = float(os.environ.get("HUMANIZER_W_DET", 1.0))
    w_sim: float = float(os.environ.get("HUMANIZER_W_SIM", 0.4))
    w_pat: float = float(os.environ.get("HUMANIZER_W_PAT", 0.3))
    sim_floor: float = float(os.environ.get("HUMANIZER_SIM_FLOOR", 0.78))
