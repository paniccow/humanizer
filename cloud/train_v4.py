...
VRAM management for 3B in bf16 on 24GB 4090:
  Qwen-3B bf16:           ~6.0 GB
  LoRA grads + optim:     ~1.5 GB
  Reference (via .disable_adapter()):  0
  3 detectors bf16:       ~3.0 GB
  Sim model (MiniLM):     ~0.4 GB
  Generation buffers G=4 × completion=180:  ~5 GB
  Activations (grad ckpt): ~5 GB
  Total:                 ~21 GB. Tight, fits.
...
class Cfg:
    ...
    max_completion_length: int = 180          # tighter for VRAM with 3B model
