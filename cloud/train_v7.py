"""GRPO inner loop v7 — single hard detector + perplexity fluency reward.

Direct response to the Run #6 dense regression. Run #6 dense (Pangram in
the reward, every 2 steps) made the policy WORSE on Pangram (0/10 vs
1/10 base). The signal we got: averaging an easy detector (roberta-base,
93% ASR — basically broken) with a hard detector (roberta-large, 10%
ASR) gave the policy lazy gradients — fool the easy one, ignore the
hard one.

Recipe (changes from v6):
  - DROP roberta-base. Single detector: roberta-large-openai-detector.
    Harder, more accurate. Forces the policy to actually learn.
  - DROP Pangram from training reward. Spent $21 confirming Pangram-in-
    loop doesn't help. Save credits for inference-time validation.
  - ADD perplexity reward computed via the BASE model (.disable_adapter()).
    Base Qwen-3B is the natural-language reference. If the policy's output
    has high perplexity under base, the policy is generating gibberish to
    chase the detector reward — penalize that.

  R(y) = w_det · (1 - p_ai_roberta_large)         # single hard detector
       + w_perp · perplexity_score(y; base)        # fluency guard
...
perplexity_score = 1 / (1 + perplexity / 50). perplexity ≈ 5 → ~0.91,
perplexity ≈ 50 → 0.5, perplexity ≈ 1000 → ~0.05. Bound [0, 1].

VRAM budget on 22GB-effective 4090:
  Qwen-3B bf16:                ~6.0 GB
  LoRA-r8 grads + optim:       ~1.0 GB
  Reference (.disable_adapter()):  0
  1 frozen detector (large):   ~0.7 GB
  Sim model (MiniLM):          ~0.4 GB
  Gen buffers G=2:             ~2.5 GB
  Activations:                 ~5 GB
  Total:                       ~15 GB. Plenty of headroom.

ORIGINAL v6 docstring (kept for reference):
...
class Cfg:
    ...
    # v7: single hard detector. roberta-base was 93% ASR (basically broken)
    # in Run #6 dense — averaging it with roberta-large diluted the signal.
    ...
    # Sparse Pangram calling. v7 default: 0 (Pangram disabled — Run #6
    # proved Pangram-in-loop doesn't help and burns credits).
    pangram_every: int = int(os.environ.get("HUMANIZER_PANGRAM_EVERY", 0))
    # Perplexity reward (v7 addition). Reward fluency under the base
    # model (.disable_adapter()) so the policy doesn't degenerate into
    # gibberish chasing the detector reward.
    use_perplexity_reward: bool = os.environ.get("HUMANIZER_USE_PERP", "1") == "1"
    perplexity_threshold: float = 50.0   # perplexity at which reward = 0.5
    ...
    # v7 perplexity reward weight. 0.5 = mid-strength fluency guard.
    # Sigmoid-like score in [0,1], so this contributes up to +0.5 to R.
    w_perp: float = float(os.environ.get("HUMANIZER_W_PERP", 0.5))
...
def perplexity_under_base(policy, tokenizer, candidates: list[str]) -> list[float]:
    """Compute perplexity of each candidate under the BASE model
    (.disable_adapter() context — no LoRA, just Qwen-3B).

    Returns list of float perplexity values. Lower = more fluent / natural
    under the base distribution. ~5-10 for typical paraphrase, >100 for
    gibberish. We map this to a [0,1] reward in compute_reward.

    No backprop (torch.no_grad). One forward pass per candidate, batched.
    """
    import math
    import torch
    ...
    with torch.no_grad():
        with policy.disable_adapter():
            for text in candidates:
                if not text or not text.strip():
                    perps.append(1000.0)  # max-penalty for empty
                    continue
                ...
                perp = math.exp(-mean_logp)
                # Clamp to avoid inf for outlier outputs
                perps.append(min(perp, 1e4))
                del out, logits, log_probs, gathered
    return perps


def perplexity_to_reward(perp: float, threshold: float = 50.0) -> float:
    """Map perplexity -> [0, 1] reward. perp=5 -> ~0.91, perp=50 -> 0.5,
    perp=1000 -> ~0.05. Smooth, bounded, monotonic-decreasing.
    """
    return 1.0 / (1.0 + perp / threshold)
...
def compute_reward(cfg, sim_model, detectors, source, candidates, *,
                    pangram_api_key=None, policy=None, tokenizer=None):
    """Reward includes the local detectors + (optional) perplexity-under-base
    fluency reward + sim + pattern + length.

    policy + tokenizer required for perplexity reward (cfg.use_perplexity_reward).
    Pass policy=None to skip perplexity (back-compat with v6 callers)."""
    import torch
    ...
    # v7: perplexity reward (one extra forward pass per candidate under
    # the base model — torch.no_grad, cheap relative to detector calls)
    if cfg.use_perplexity_reward and policy is not None and tokenizer is not None:
        perps = perplexity_under_base(policy, tokenizer, candidates)
        perp_rewards = [perplexity_to_reward(p, cfg.perplexity_threshold) for p in perps]
    else:
        perps = [0.0] * len(candidates)
        perp_rewards = [0.0] * len(candidates)

    rewards: list[float] = []
    details: list[dict] = []
    for c, p, s, perp, perp_r in zip(candidates, p_ais, sims, perps, perp_rewards):
        ...
        if s < cfg.sim_floor:
            ...
            details.append({
                ...
                "perplexity": perp, "perp_reward": perp_r,
                "reward": r, "gated": True,
            })
        else:
            r = (
...
def train(cfg: Cfg) -> tuple[list[str], list[dict]]:
    ...
    # v7: Pangram is OPTIONAL. Only required if cfg.pangram_every > 0.
    pangram_api_key = os.environ.get(cfg.pangram_api_key_env)
    if cfg.pangram_every > 0:
        if not pangram_api_key:
            raise RuntimeError(
                f"pangram_every={cfg.pangram_every} but {cfg.pangram_api_key_env} not set."
            )
        print(f"[pangram] api key present, will fire every {cfg.pangram_every} steps", flush=True)
    else:
        print("[pangram] disabled (pangram_every=0); training against local detectors only", flush=True)
    print(f"[v7] perplexity reward: {'on' if cfg.use_perplexity_reward else 'off'}  "
          f"w_perp={cfg.w_perp}  threshold={cfg.perplexity_threshold}", flush=True)
    ...
    for step, src in enumerate(train_prompts):
        ...
        line = {
            ...
            "mean_perplexity": float(sum(d.get("perplexity", 0.0) for d in details) / len(details)),
            "mean_perp_reward": float(sum(d.get("perp_reward", 0.0) for d in details) / len(details)),
        ...
        print(
            ...
            f"pat={line['mean_pattern']:.3f}  perp={line['mean_perplexity']:.1f}  "
            f"gated={line['n_gated']}/{cfg.num_generations}  "
